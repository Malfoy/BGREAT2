...
uint Aligner::mapOnLeftEndExhaustive(const string &read, vector<uNumber>& path, const pair<kmer, uint>& overlap , uint errors){
	...
	vector<uNumber> path2keep;
	if(readLeft.size()==0){path.push_back(0);return 0;}
...
uint Aligner::checkBeginExhaustive(const string& read, const pair<kmer, uint>& overlap, vector<uNumber>& path, uint errors){
...
uint Aligner::checkEndExhaustive(const string& read, const  pair<kmer, uint>& overlap, vector<uNumber>& path, uint errors){
